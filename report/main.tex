...
Using Spectrum-Based Methods and an MLP Baseline}
...
The critical methodological issue is \emph{position}. A detector that only examines the beginning of a word is valid for \textit{d} in \textit{dog}, but not for \textit{d} in \textit{bed} or \textit{r} in \textit{zero}. Therefore, we study position-independent detection of four voiced consonants:
\begin{itemize}
...
\begin{enumerate}
    ...
    \item How does an MLP classifier trained directly on whole-word features compare with the classical detectors?
\end{enumerate}
...
Our comparison therefore includes MATLAB spectrum methods based on FFT and STFT, MFCC/DTW baselines, and an MLP trained on the same Speech Commands recordings.
...
For the MLP method, each full word is converted into a resized log-Mel spectrogram vector $z(x)$. We train one binary MLP per target consonant:
\begin{equation}
...
Figure~\ref{fig:perletter} compares one MFCC/DTW baseline, one local-spectrum classical detector (STFT k-NN), and the MLP on each letter. In the test results, the MLP obtains the highest F1 for all four consonants:
\begin{itemize}
...
A detector restricted to the word onset would miss valid cases such as \textit{d} in \textit{bed}, \textit{n} in \textit{seven}, or \textit{r} in \textit{zero}. The full-word scan avoids that error by evaluating the entire active region.
...
\textbf{3) MFCC/DTW baselines and spectrum baselines are both informative.}
...
\textbf{4) The MLP gives the highest scores in this study.}
...
\section{Conclusion}
This work analyzed the spectra of four voiced consonants in MATLAB, implemented MATLAB spectrum-based detectors, evaluated MFCC/DTW baselines, and trained an MLP on Speech Commands recordings. The task is formulated as \emph{position-independent} letter detection inside words rather than onset-only classification.
