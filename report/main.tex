...
Six of these keywords, \textit{cat}, \textit{go}, \textit{house}, \textit{up}, \textit{wow}, and \textit{yes}, do not contain any of the four target consonants and are included as target-free negative examples. To keep the computation comparable across all methods, each keyword is capped at 150 training recordings, 60 validation recordings, and 120 test recordings, giving 6,270 recordings in total. Speaker-independent splits use the hexadecimal filename token before \texttt{\_nohash\_}: after conversion to an integer and modulo 100, buckets 0--19, 20--29, and 30--99 are test, validation, and training, respectively, giving about 70\%/10\%/20\% train/validation/test before caps.
...
\begin{figure}[t]
    ...
    \fbox{\parbox{0.88\linewidth}{\centering Input waveform $\rightarrow$ \textbf{Algorithm 1}: normalize, find active speech, form features}}\\[-0.5mm]
    \fbox{\parbox{0.88\linewidth}{\textbf{A. Whole-word template}: global FFT; train positive mean FFT; test by cosine threshold.}}\\[-0.5mm]
    \fbox{\parbox{0.88\linewidth}{\textbf{B. Local template scan}: Algorithms 2--3; MFCC mean, MFCC + DTW, STFT mean, STFT k-NN; train target banks, then scan all windows.}}\\[-0.5mm]
...
The main limitation is that target timing is approximate rather than forced-aligned, and the balanced design uses only four positive keyword types per target consonant.
