...
\begin{figure}[t]
    ...
    $\downarrow$\\[-1mm]
    \fbox{\parbox{0.88\linewidth}{\centering \textbf{Algorithm 1}\\normalize, find active speech, form features}}\\[1mm]
    $\downarrow$\\[-1mm]
    \emph{Choose one method branch}\\[1mm]
    \fbox{\parbox{0.88\linewidth}{\textbf{A. Whole-word template}: global FFT; train positive mean FFT; test by cosine threshold.}}\\[1mm]
    \fbox{\parbox{0.88\linewidth}{\textbf{B. Local template scan}: Algorithms 2--3; MFCC mean, MFCC + DTW, STFT mean, STFT k-NN; train target banks, then scan all windows.}}\\[1mm]
    \fbox{\parbox{0.88\linewidth}{\textbf{C. Learned full-word classifier}: Algorithm 4; MLP on full-word log-Mel features.}}
...
The algorithmic structure is summarized in Fig.~\ref{fig:methodmap}. Algorithm~1 is common preprocessing: waveform normalization, active-speech detection, and generation of full-word and local-window features. Algorithm~2 builds classical templates: the global FFT method averages positive whole-word spectra, while the local methods build target-window banks using the approximate phoneme-position prior in (4). Algorithm~3 scores classical test recordings by thresholding either the whole-word FFT cosine score or the best local-window cosine, DTW, or k-NN score. Algorithm~4 trains one standardized full-word log-Mel MLP per target consonant and thresholds its output probability.
