...
Our main contribution is a full-word comparison on Speech Commands recordings. Local target templates are constructed from approximate phoneme-position priors, and each local detector scans the entire active part of the word at test time.
...
Figure~\ref{fig:examples} is equally important for validating the task definition. The highlighted windows include different target positions: \textit{d} is captured at the end of \textit{bed}, \textit{n} at the end of \textit{seven}, \textit{r} in the middle of \textit{zero}, and \textit{v} at the beginning of \textit{visual}. These examples illustrate the full-word scanning problem.
...
The target consonants occur in initial, medial, and final positions across the selected keywords. The full-word scan evaluates the entire active region instead of assigning a single fixed target position.
...
There are still limitations. The target timing is approximate rather than forced-aligned, and the balanced design uses only four positive keyword types per target consonant. A phoneme-aligned corpus would allow a cleaner study of purely acoustic consonant spotting.
